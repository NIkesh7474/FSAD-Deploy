...
export function AppProvider({ children }) {
    ...
    const handleException = (action, error) => {
        ...
        // Silently collect the error without exposing it in the UI for cleaner deployment behavior.
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                handleException('API fetch', error)
            }
        }
    ...
    const removeUser = async (id) => {
        try {
            ...
            setApiError(response.data.message || 'Failed to remove user')
        } catch (error) {
            handleException('Remove User', error)
        }
        return false
    }
    ...
    const updateUserRole = async (id, role) => {
        try {
            ...
            setApiError(response.data.message || 'Failed to update user role')
        } catch (error) {
            handleException('Update User Role', error)
        }
        return false
    }
    ...
    const approveHomestay = async (id) => {
        ...
        if (hs) await createHomestay(hs)
        setPendingHomestays(prev => prev.filter(h => h.id !== id))
    }
    const rejectHomestay = (id) => setPendingHomestays(prev => prev.filter(h => h.id !== id))

    const addPendingGuide = (g) => setPendingGuides([...pendingGuides, { ...g, id: Date.now() }])
    const approveGuide = async (id) => {
        const guide = pendingGuides.find(x => x.id === id)
        if (guide) await createGuide(guide)
        setPendingGuides(prev => prev.filter(x => x.id !== id))
    }
    const rejectGuide = (id) => setPendingGuides(prev => prev.filter(x => x.id !== id))
    ...
    const createHomestay = async (homestayData) => {
        try {
            ...
            setApiError(response.data.message || 'Failed to create homestay')
        } catch (error) {
            handleException('Create Homestay', error)
        }
        return false
    }
    ...
    const updateHomestay = async (id, updates) => {
        try {
            ...
            setApiError(response.data.message || 'Failed to update homestay')
        } catch (error) {
            handleException('Update Homestay', error)
        }
        return false
    }
    ...
    const deleteHomestay = async (id) => {
        try {
            ...
            setApiError(response.data.message || 'Failed to delete homestay')
        } catch (error) {
            handleException('Delete Homestay', error)
        }
        return false
    }
    ...
    const createTouristPlace = async (placeData) => {
        try {
            ...
            setApiError(response.data.message || 'Failed to create tourist place')
        } catch (error) {
            handleException('Create Tourist Place', error)
        }
        return false
    }
    ...
    const updateTouristPlace = async (id, updates) => {
        try {
            ...
            setApiError(response.data.message || 'Failed to update tourist place')
        } catch (error) {
            handleException('Update Tourist Place', error)
        }
        return false
    }
    ...
    const deleteTouristPlace = async (id) => {
        try {
            ...
            setApiError(response.data.message || 'Failed to delete tourist place')
        } catch (error) {
            handleException('Delete Tourist Place', error)
        }
        return false
    }
    ...
    const createGuide = async (guideData) => {
        try {
            ...
            setApiError(response.data.message || 'Failed to create guide')
        } catch (error) {
            handleException('Create Guide', error)
        }
        return false
    }
