...
export default function AdminPortal() {
    const { currentUser, users, removeUser, updateUserRole, bookings, pendingHomestays, approveHomestay, rejectHomestay, pendingGuides, approveGuide, rejectGuide } = useContext(AppContext);
                        ...
                        {users.map(u => (
                            <div key={u.id} style={{ padding: '1.5rem', background: 'white', borderRadius: '0.5rem', border: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div>
                                    ...
                                    <span style={{ color: 'var(--text-light)', marginLeft: '0.5rem' }}>({u.email})</span>
                                    <div style={{ marginTop: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                        <label style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-dark)' }}>Platform Role:</label>
                                        <select 
                                            value={u.role} 
                                            onChange={(e) => updateUserRole(u.id, e.target.value)}
                                            style={{ padding: '0.25rem 0.5rem', borderRadius: '0.25rem', border: '1px solid #d1d5db', outline: 'none', background: 'var(--white)', cursor: 'pointer' }}
                                        >
                                ...
                                <div style={{ display: 'flex', gap: '0.75rem' }}>
                                    ...
                                    <button className="btn" style={{ background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca' }} onClick={() => {
                                        if (window.confirm(`Are you sure you want to completely remove ${u.name} from the platform?`)) {
                                            removeUser(u.id);
                                        }
                                    }}>Remove User</button>
                                </div>
